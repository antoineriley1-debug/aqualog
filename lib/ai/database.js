 ...
 * (c) 2026 Antoine Riley
 */

import { createClient } from '@supabase/supabase-js';

// Lazy Supabase client: created only when a function actually runs at
// request time -- NOT at module load. This prevents "supabaseUrl is required"
// build failures when env vars are not present during `next build`.
let _client = null;
function supa() {
...
export async function logUsage(userId, model, taskType, tokens, cost, duration, status = 'success', errorMessage = null) {
  try {
    const { error } = await supa().from('ai_usage_logs').insert({
      user_id: userId,
      model,
      task_type: taskType,
