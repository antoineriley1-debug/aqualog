/**
 * FacilityH2O — Shared Notification Layer (Email + SMS)
...
export async function sendSMS(numbers, message) {
  ...
  if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_FROM_NUMBER) {
    return { ok: false, skipped: true, error: 'Twilio not configured', recipients: nums };
  }
  if (!nums.length) {
    return { ok: false, skipped: true, error: 'No SMS numbers', recipients: [] };
  }
  ...
  try {
    ...
    for (const num of nums) {
      try {
        await client.messages.create({ body: message, from: process.env.TWILIO_FROM_NUMBER, to: num });
        sent.push(num);
      } catch (e) {
        failed.push({ num, error: e.message });
        console.error('[notify:sms] FAILED for', num, ':', e.message);
      }
    }
    return { ok: sent.length > 0, recipients: sent, failed: failed.length ? failed : undefined };
  } catch (err) {
    console.error('[notify:sms] FAILED:', err.message);
    return { ok: false, error: err.message, recipients: nums };
  }
}
