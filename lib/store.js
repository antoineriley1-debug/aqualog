...
const FEATURE_KEYS = ['boiler','chilled','st108','legionella','coc','advisor','reports','compliance','api_access','custom_equipment'];
...
/**
 * Does this user's organization have the Enterprise "specialized equipment library" feature?
 * Owner (ariley) always has it. Otherwise it must be in the org's active license features.
 * This gates the curated custom-equipment library (steam sterilizers, dialysis water, etc.).
 */
export function hasCustomEquipmentFeature(user) {
  if (!user) return false;
  if (user.username === 'ariley' || user.id === 'usr_ariley') return true;
  if (!user.orgId) return false;
  const lic = (getLicenses() || []).find(l => l.orgId === user.orgId);
  if (!lic) return false;
