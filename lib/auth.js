...
const FALLBACK_USERS = []; // production: all users live in data/users.json — no credentials in source. Emergency recovery: restore a user object into data/users.json and redeploy.
...
/**
 * readUsers() — environment-first so authentication can never drift on an
 * ephemeral host (Render resets the container filesystem on every restart).
 *
 *   AUTH_USERS_JSON  — optional. Full users array as JSON; if set, it is the
 *                      source of truth and the disk file is ignored.
 *   ADMIN_PASSWORD   — optional. Overrides the admin (ariley) password only.
 *                      Lets you rotate the admin login from the Render dashboard
 *                      with zero code changes and zero filesystem dependence.
 *
 * If neither is set, falls back to data/users.json (original behavior).
 */
function readUsers() {
  // 1) full override
  if (process.env.AUTH_USERS_JSON) {
    try {
      const envUsers = JSON.parse(process.env.AUTH_USERS_JSON);
      if (Array.isArray(envUsers) && envUsers.length) return envUsers;
    } catch (e) {
      console.error('AUTH_USERS_JSON is set but not valid JSON — ignoring:', e.message);
    }
  }

  // 2) start from disk (or empty)
  let users = fileUsers();
  if (!users.length) users = FALLBACK_USERS.slice();

  // 3) admin password override — applied no matter where the base list came from
  const adminPw = process.env.ADMIN_PASSWORD;
  if (adminPw) {
    const adminUser = process.env.ADMIN_USERNAME || 'ariley';
    const idx = users.findIndex((u) => u.username === adminUser);
    if (idx >= 0) {
      users[idx] = { ...users[idx], password: adminPw, active: true };
    } else {
      users.push({
        id: 'usr_admin', username: adminUser, password: adminPw, role: 'admin',
        hospital: null, name: 'Administrator', email: process.env.ADMIN_EMAIL || '', active: true,
